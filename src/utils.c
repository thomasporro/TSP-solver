...
void generate_random_solution(instance *inst, int *node_list) {
    ...
    double cost = distance(node_list[0], node_list[inst->nnodes - 1], inst);
    for (int i = 0; i < inst->nnodes - 1; i++) {
        cost += distance(node_list[i], node_list[i + 1], inst);
    }
...
void list_to_successors(instance *inst, int *node_list, int *successors) {
    // Last to first
    successors[node_list[inst->nnodes - 1]] = node_list[0];
    for (int i = 0; i < inst->nnodes - 1; i++) {
